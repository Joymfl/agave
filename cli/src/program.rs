use {
    crate::{
        ...
        signal_handler::{cancel_token, run_until_interrupted},
    },
...
/// A [`WorkersBroadcaster`] that applies backpressure: it `.await`s each send
/// instead of dropping the batch when a worker's channel is full (as the
/// default [`NonblockingBroadcaster`] does), so the producer is paced to what
/// the QUIC workers can absorb rather than silently discarding transactions
/// under load.
///
/// Copied from the `BackpressuredBroadcaster` in
/// `client-test/tests/send_and_confirm_transactions_in_parallel.rs`. The
/// `tpu-client-next` crate ships only the dropping `NonblockingBroadcaster` and
/// leaves backpressure for each consumer to implement (see anza-xyz/agave#13799).
///
/// Leaders in the fanout are served sequentially, so a slow leader delays
/// delivery to the rest of the batch.
///
/// [`WorkersBroadcaster`]: solana_tpu_client_next::connection_workers_scheduler::WorkersBroadcaster
